...
/**
 * generate_50_leads.js — Batch pipeline for 50 trade business leads in Riga
 *
 * Pipeline:
 *   1. Scrape 60+ trade businesses from Google Maps Riga via Apify
 *   2. Find emails via Hunter.io + page scraping fallback
 *   3. Screenshot websites (skip if no website)
 *   4. Analyze websites with OpenAI Vision
 *   5. Generate personalized cold emails in Latvian
 *   6. Output: markdown report + CSV (NO emails are sent)
 *
 * Usage:
 *   node scripts/generate_50_leads.js
 */
...
const SCREENSHOT_DIR = path.join(PROJECT_DIR, "screenshots", "trades_batch");

const TARGET_COUNT = 50;

// Trade-specific search queries for Riga
const SEARCH_QUERIES = [
    "santehniķis Rīga",           // plumber
    "jumta remonts Rīga",          // roofer
    "autoserviss Rīga",            // auto mechanic
    "elektriķis Rīga",             // electrician
    "remontdarbi Rīga",            // general repairs / handyman
    "siltināšana Rīga",            // insulation
    "grīdas ieklāšana Rīga",       // flooring
    "apkures sistēmas Rīga",       // heating systems
    "logu montāža Rīga",           // window installation
    "kondicionēšana Rīga",         // HVAC / AC
];
...
const TRADE_ANALYSIS_PROMPT = `You are analyzing a screenshot of a local trade/service business website in Latvia (plumber, roofer, mechanic, electrician, handyman, HVAC, or similar trade).
...
3. No service list or price information visible on the page
4. No photos of completed work or projects, or photos are low quality
5. No business hours or availability shown
6. No clear contact form — only a phone number, no way to request a quote online
7. No customer reviews or testimonials visible
8. No address or service area information visible
...
- mājaslapā nav norādīts pakalpojumu saraksts vai cenas — klienti nevar novērtēt piedāvājumu pirms zvanīšanas
- mājaslapā nav darbu fotogrāfiju — klienti nevar redzēt iepriekšējo projektu kvalitāti
- mājaslapā nav tiešsaistes pieteikuma formas — klienti, kas negrib zvanīt, vienkārši aiziet pie konkurentiem
...
const TRADE_EMAIL_PROMPT = `# SYSTEM PROMPT — Latvian B2B Cold Email Generator (Trade Businesses)

## ROLE

You are a native Latvian B2B copywriter specializing in cold email outreach for local trade and service businesses (plumbers, roofers, mechanics, electricians, handymen, HVAC companies). You write short, formal, grammatically flawless Latvian business emails that are deeply personalized, culturally appropriate, and legally compliant.
...
1. Greeting — "Labdien," (no name since we don't know it for trades)
...
3. Who you are + what you do — One sentence. "Es specializējos tieši mazo pakalpojumu uzņēmumu mājaslapu izveidē Rīgā." Frame as specialist, never say "we".
4. Social proof — COPY-PASTE VERBATIM: "Nesen palīdzēju Rīgas remonta uzņēmumam izveidot jaunu mājaslapu, un viņi sāka saņemt tiešsaistes pieprasījumus jau pirmajā nedēļā."
5. CTA — COPY-PASTE VERBATIM: "Vai Jums būtu interese — varu nosūtīt īsu video, kurā parādu, kā moderna mājaslapa varētu izskatīties Jūsu uzņēmumam?"
...
Reference that they currently don't have a website, and that potential clients searching online for their services in Riga are finding competitors instead. Tie this to concrete lost revenue — clients searching Google for "santehniķis Rīga" or their specific trade + Rīga will not find them.

### For businesses WITH a website (has_website = true):
Reference the specific problem found from the website analysis. Make it concrete — not "your website is old" but the exact issue and its consequence.
...
async function main() {
    console.log("═══════════════════════════════════════════════════════════");
    console.log("  Cold Email Pipeline — 50 Trade Businesses in Riga");
    ...
    for (let i = 0; i < leads.length; i++) {
        ...
        try {
            ...
            if (lead.hasWebsite) {
                ...
                const tradeType = lead.category || "pakalpojumu sniedzējam";
                problem = `uzņēmumam nav mājaslapas — klienti, kas meklē ${tradeType.toLowerCase()} Rīgā, Jūs neatrod un izvēlas konkurentus`;
            }
    ...
    // Markdown report
    let md = `# 📧 50 Trade Business Cold Emails — Riga\n\n`;
    ...
    const reportPath = path.join(PROJECT_DIR, "50_trade_leads_report.md");
    ...
    const csvPath = path.join(PROJECT_DIR, "50_trade_leads.csv");
